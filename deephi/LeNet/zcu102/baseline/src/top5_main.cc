...
/**
 * @brief Run CONV Task for LeNet
 *
 * @param taskConv - pointer to LeNet CONV Task
 *
 * @return none
 */
...
#define DPU_MODE_DUMP   2	    
        ...
        for(unsigned int ind = i  ;ind < images.size();ind+=threadnum)
	{
	    ...
	    //Mat img2; resize(img, img2, sz); //DB
            //run_LeNet(taskconv,img2); //DB: images are already 32x32 and do not need any resize
	    run_LeNet(taskconv,img);
	}
